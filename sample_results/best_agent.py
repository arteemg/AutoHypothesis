...
This harness achieved 1.25 Sharpe in-sample and 0.86 on the holdout set. It took approx 4 hours to run with Claude Opus.
...
def get_regime(data: dict) -> pd.Series:
    ...
    bull = (spy > spy_200d) & (spy_50d > spy_200d)
    regime = pd.Series(np.where(bull, 1.0, 0.5), index=spy.index)
    return regime
...
def get_signals(data: dict) -> pd.DataFrame:
    ...
    # Volume trend: 60d avg volume / 200d avg volume — rising = institutional accumulation
    vol_60d = volume.rolling(60).mean()
    vol_200d = volume.rolling(200).mean()
    vol_trend = vol_60d / vol_200d.replace(0, np.nan)
    vol_trend_rank = vol_trend.rank(axis=1, pct=True)

    # Inverse volatility: low-vol anomaly (Ang et al. 2006)
    inv_vol = -daily_ret.rolling(60).std()
    inv_vol_rank = inv_vol.rank(axis=1, pct=True)

    composite = (0.75 * vol_trend_rank + 0.25 * inv_vol_rank)
    composite = composite.ewm(span=252).mean()

    # Top 20% by composite
    threshold = composite.quantile(0.80, axis=1)
    signals = composite.gt(threshold, axis=0)
    return signals
...
def get_position_sizes(signals: pd.DataFrame, data: dict) -> pd.DataFrame:
    ...
    n_stocks = signals.sum(axis=1).replace(0, np.nan)
    eq_weight = signals.astype(float).div(n_stocks, axis=0).fillna(0.0)

    weights = eq_weight.mul(regime, axis=0)
    return weights
